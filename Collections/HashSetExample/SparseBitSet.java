...
{ 
	...
	public int length()
	{
	int returnValue = 0; // Make sure set not empty // Get maximum value +1
	if (bitset.size() > 0) 
	{
	...
	{
	Set thisSet = new HashSet(bitset); 
	Set thatSet = new HashSet(set.getBitSet());

	// Find out what elements are in this set but not in the other set 
	thisSet.removeAll(thatSet);

	// Find out what elements are in the other set but not in this set
	thatSet.removeAll(bitset);
	
	// Combine sets 
	bitset.retainAll(thisSet);
	bitset.addAll(thatSet);
	}
}